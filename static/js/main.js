...
$(function () {
  ...
  wow.init();
  $('.mobile__popup').append($('header .logo').clone());
  $('.mobile__popup').append($('header nav').clone());
  $('.mobile__popup .mobile__popup-buttons').append($('header .lang').clone());
  $('.mobile__popup .mobile__popup-buttons').append($('header .tell').clone());
  $('.mobile__popup').append($('header .callme').clone());
  $('.mobile__popup .mobile__popup-buttons').append($('header .profile').clone());
  ...
  if (window.location.hash && $(window.location.hash).is(window.location.hash)) {
  ...
  $('body').on('click', '.mainbanner__down', function () {
    $('.scrolling-point').each(function () {
      var scrollingEl = $(this).offset().top - 90;

      if ($(document).scrollTop() < scrollingEl) {
        $('body, html').animate({
          scrollTop: scrollingEl
        }, 500);
        return false;
      }
    });
  });
  $('body').on('click', '.mobile__popup .dropdown', function () {
    if ($(this).is('.active')) {
      $(this).removeClass('active');
      $(this).find('.dropdown__links').slideUp();
    } else {
      $(this).addClass('active');
      $(this).find('.dropdown__links').slideDown();
    }
  });
  ...
  $('.galleryslider').each(function () {
    var slider = $(this).find('.galleryslider__container');
  ...
  $('.mainslider').each(function () {
    ...
    var left = $(this).find('.left');
    var right = $(this).find('.right');
    var paggination = $(this).find('.swiper-pagination');
    var mySwiper = new Swiper(slider, {
      slidesPerView: 2,
      spaceBetween: 30,
      navigation: {
  ...
  $('.blog-mobile-slider').each(function () {
    var slider = $(this).find('.swiper-container');
    var paggination = $(this).find('.swiper-pagination');
    var mySwiper = new Swiper(slider, {
      slidesPerView: 1,
  ...
  $('.galleryslider__container-mobile').each(function () {
    var slider = $(this).find('.swiper-container');
    var paggination = $(this).find('.swiper-pagination');
    var mySwiper = new Swiper(slider, {
      slidesPerView: 1,
  ...
  $('body').on('click', '[data-popup-open]', function () {
    ...
    if ($('.' + popupClass).is('.' + popupClass)) {
      if ($('.popup').is('.active') || $('.mobile__popup').is('.active')) {
        $('.popup').removeClass('active');
        $('.mobile__popup').removeClass('active');
        $('.' + popupClass).addClass('active');
      } else {
        var widthBody = $('body').width();
        $('body, header').css('maxWidth', widthBody + 'px');
        $('.' + popupClass).addClass('active');
  ...
  $('body').on('click', '.popupClose', popupClose);
  animateSvg('.svg-animate-selector', 3000);
});
...
function animateSvg(selector, time) {
  $(selector).each(function (index) {
    var element = $(this);
    var lastCoords = [element.offset().top - 150, index % 2 === 0 ? element.offset().left - 150 : element.offset().left + 150];
    var h = element.offset().top - 150;
    var w = index % 2 === 0 ? element.offset().left - 150 : element.offset().left + 150;

    function makeNewPosition() {
      var nh = randomInteger(h, h + 300);
      var nw = 0;

      if (index % 2 === 0) {
        nw = lastCoords[1] === w ? w + 300 : w;
      } else {
        nw = lastCoords[1] === w ? w - 300 : w;
      }
...
function initPopupSlider() {
  var slider = $('.popup-slider').find('.swiper-container');
  var left = $('.popup-slider').find('.left');
  var right = $('.popup-slider').find('.right');
  popupSlider = new Swiper(slider, {
    spaceBetween: 10,
    navigation: {
      nextEl: right,
      prevEl: left
...
function openPopupReviews(data) {
  var popupClass = 'popup__review';
  var slider = $('.popup__review .swiper-wrapper');

  if (data.name) {
    $('.popup-author-info span').html(data.name);
  }

  if (data.studying) {
    $('.popup-author-info p').html(data.studying);
  }

  if (data.profilePhoto.url) {
    $('.popup-author-photo img').attr('src', data.profilePhoto.url);
  }

  if (data.profilePhoto.alt) {
    $('.popup-author-photo img').attr('alt', data.profilePhoto.alt);
  }

  if (data.text) {
    $('.popup-author-text').html(data.text);
  }

  slider.html('');
  data.images.forEach(function (item) {
    slider.append("<div class=\"swiper-slide\">\n\t\t\t\t<img src=\"".concat(item.url, "\" alt=\"").concat(item.alt, "\">\n\t\t\t</div>"));
  });
